...
navbar = dbc.NavbarSimple(
    children=[dbc.NavItem(dbc.NavLink("Models", href="/add_model"))],
...
network_graphs = dbc.Row(
    [dbc.Col([network], sm=12, md=6), dbc.Col([network_graph], sm=12, md=6)]
)
    ...
    [
        dbc.Col(
            ...
            """
                ...
                html.Button(id="update-models-dropdown-btn", style={"display": "none"}),
                node_graph,
            ],
            sm=12,
            md=6,
...
def get_model_features_components(model_name, node):
    ...
    if model_name == node["model"]:
        node_features = [
            str(f) for f in node["parameters"] + node["initial-conditions"]
        ]
        ...
        result.children.append(
            dbc.Row(
                children=[
                    ...
                    dbc.Col(
                        [dbc.Input(id=f"{feat}-input", type="text", value=value)], md=7
                    ),
                ],
                style={"margin": "15px"},
            )
...
def update_model_list(
    n_clicks,
):  # Updates the models in the dropdown when reloading the page
